...
spark = (
    ...
    .config("spark.sql.execution.arrow.pyspark.enabled", "false")  # Tắt Arrow
    .getOrCreate()
)
...
def absa_infer_udf(texts: pd.Series) -> pd.Series:
    # 🔧 DUMMY MODEL - để test infrastructure
    # TODO: Sau khi test xong, có thể enable lại model thật bằng cách:
    #   1. Tăng Docker memory limit lên 8GB
    #   2. Uncomment code bên dưới
    #   3. Comment phần dummy này
    
    # === DUMMY IMPLEMENTATION ===
    results = []
    for _ in texts:
        p_m = [0.5] * len(ASPECTS)          # uniform aspect probability
        p_s = [0.33] * (len(ASPECTS)*3)     # uniform sentiment (POS/NEU/NEG)
        results.append(p_m + p_s)
    return pd.Series(results)
    
    # === REAL MODEL (COMMENTED FOR NOW) ===
...
def write_to_postgres(batch_df, batch_id):
    ...
    # Bỏ crash simulation để chạy liên tục
    # if batch_id % 5 == 0:
